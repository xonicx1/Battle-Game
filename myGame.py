...
#load images
#background image
background_img = pygame.image.load('img/Background/background2.png').convert_alpha()
#panel image
...
class Fighter():
    ...
    def update(self):
        ...
        if pygame.time.get_ticks() - self.update_time > animation_cooldown:
            self.update_time = pygame.time.get_ticks()
            self.frame_index += 1
        #if the animation has run out then reset to start
        ...
        if knight.alive == True:
            if current_fighter == 1:
                ...
                if action_cooldown >= action_wait_time:
                    ...
                    if potion == True:
                        if knight.potions > 0:
                            #check if potion will heal beyond max health
